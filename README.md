...
│   ├── binary_model.py      # Trains the Random Forest Classifier (Bedrock vs. Soil) using Optuna.
│   ├── qrf_model.py         # Trains the Quantile Regression Forest (Depth to bedrock prediction & Uncertainty).
│   └── fusion.py            # Implements the Hurdle Model logic (overrides depth with 0 where P(Bedrock) > Threshold).
├── optimization/            # Hyperparameter tuning
│   └── objectives.py        # Defines Optuna objective functions
├── visualization/           # Plotting utilities
│   └── plots.py             # Generates figures 
├── config.py                # Central configuration: Defines file paths, feature columns, and uncertainty quantiles.
├── data_utils.py            # Preprocessing: Handles data loading, stratified subsampling, and directory creation.
├── evaluation.py            # Validation Workflow: runs comprehensive assessment (RMSE, Bias, PICP) by depth class.
├── metrics.py               # Metric Definitions: implementations of CCC, Interval Coverage etc..
├── main.py                  # Training: load CSVs, train models, optimize hyperparameters, and save artifacts.
├── prediction.py            # Inference: Applies trained models to Raster/GeoTIFF tiles to generate final 2m/10m map products.
...
## Note on Uncertainty Visualization: 
While prediction.py generates the fundamental uncertainty rasters (e.g., Q05, Q95, Q50), specialized visualization products such as QuadMaps are generated in separate processes. see: https://gitlab.com/spadarian/quadmap-rs
